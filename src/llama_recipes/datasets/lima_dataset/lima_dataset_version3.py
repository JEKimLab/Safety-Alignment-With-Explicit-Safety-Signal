...
class InstructionDataset(Dataset):
    def __init__(self, dataset_config, tokenizer, partition="train", max_tokens=2048):
        ...
        file_path = "/home/jli265/workspace/llama-recipes/misc/filtered_merged_dataset.jsonl"
        ...
        # 计算 0.5% 的抽样数量
        pos_sample_size = max(1, int(len(positive_parsed_data) * 0.01))  # 至少抽 1 个
        neg_sample_size = max(1, int(len(negative_parsed_data) * 0.01))  # 至少抽 1 个
    ...
    def __getitem__(self, index):
        ...
        prompt = B_INST + " " + conversation_0 + " " + E_INST
        example = prompt + " " + conversation_1 + " "

        # Tokenize `conversations[0]` separately to get specific indices
        conversation_0_tokens = self.tokenizer.encode(conversation_0, add_special_tokens=False)
        conversation_1_tokens = self.tokenizer.encode(conversation_1, add_special_tokens=False)
        ...
        start_idx_input = 4 # except cls token, sos, B_INST, space token
        end_idx_input = start_idx_input + len(conversation_0_tokens)

        # Find `conversations[1]` start and end indices in `input_ids`
        start_idx_output = end_idx_input + 3 # skip sapce, eos and space
        if ann["label"] == 1:
            end_idx_output = start_idx_output + len(conversation_1_tokens)
