...
def main(**kwargs):
    ...
    if config.model_type == "mllama":
        ...
        model = LlamaForCausalLM.from_pretrained(
            ...
            quantization_config=bnb_config,
            use_cache=use_cache,
            attn_implementation="sdpa" if train_config.use_fast_kernels else "eager",
            device_map="auto" if train_config.quantization and not train_config.enable_fsdp else None,
            torch_dtype=torch.float16 if train_config.use_fp16 else torch.bfloat16,
        )
    ...
    if not tokenizer.cls_token_id:
        print("#" * 30)
        print("#" * 30)
        print("We have to add the cls_token and resize the model's input embeddings")
        print("#" * 30)
        print("#" * 30)
    ...
    if train_config.enable_fsdp:
        ...
        mixed_precision_policy, wrapping_policy = get_policies(fsdp_config, rank, is_mistral)
