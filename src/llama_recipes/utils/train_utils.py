...
from llama_recipes.policies import fpSixteen,bfSixteen, get_llama_wrapper,get_mistral_wrapper
...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, wandb_run=None):
    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            with profile(train_config,local_rank) as profile_context:
                for step, batch in enumerate(train_dataloader):
                    ...
                    with autocast():
                        outputs, cls_logits, position_loss, language_model_loss = model(**batch)
                        loss = outputs.loss
                    total_loss += loss.detach().float()
                    total_position_loss += position_loss.detach().float()
                    total_language_model_loss += language_model_loss.detach().float()
                    loss = loss / gradient_accumulation_steps
                    position_loss = position_loss / gradient_accumulation_steps
                    language_model_loss = language_model_loss / gradient_accumulation_steps
                    if train_config.save_metrics:
                    ...
                    pbar.set_description(f"Training Epoch: {epoch+1}/{train_config.num_epochs}, step {step}/{len(train_dataloader)} completed (loss: {loss.detach().float()}, p_loss：{position_loss.detach().float()}, l_loss: {language_model_loss.detach().float()})")
        ...
        if is_xpu_available() and (torch.xpu.device_count() > 1 and train_config.enable_fsdp):
            ...
            dist.all_reduce(total_position_loss, op=dist.ReduceOp.SUM)
            dist.all_reduce(total_language_model_loss, op=dist.ReduceOp.SUM)
        elif torch.cuda.device_count() > 1 and train_config.enable_fsdp:
            dist.all_reduce(total_loss, op=dist.ReduceOp.SUM)
            dist.all_reduce(total_position_loss, op=dist.ReduceOp.SUM)
            dist.all_reduce(total_language_model_loss, op=dist.ReduceOp.SUM)
        train_epoch_loss = total_loss / len(train_dataloader)
        train_epoch_position_loss = total_position_loss / len(train_dataloader)
        train_epoch_language_model_loss = total_language_model_loss / len(train_dataloader)
        if train_config.enable_fsdp:
            train_epoch_loss = train_epoch_loss/world_size
            train_epoch_position_loss = train_epoch_position_loss/world_size
            train_epoch_language_model_loss = train_epoch_language_model_loss/world_size
        train_perplexity = torch.exp(train_epoch_language_model_loss)
        
        ############################################################# 
        ############################################################# 
        # TODO print the total position loss and language model loss
        ############################################################# 
        ############################################################# 
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer, wandb_run):
    ...
    cls_loss = 0.0
    """ 在分布式环境下计算 accuracy, precision, recall, f1 """
    total_correct = torch.tensor(0.0, device=torch.device("cuda"))  # 统计正确的预测数量
    total_samples = torch.tensor(0.0, device=torch.device("cuda"))  # 统计所有样本数量
    total_tp = torch.tensor(0.0, device=torch.device("cuda"))  # True Positives (真正例)
    total_fp = torch.tensor(0.0, device=torch.device("cuda"))  # False Positives (假正例)
    total_fn = torch.tensor(0.0, device=torch.device("cuda"))  # False Negatives (假反例) 
    ...
    with MemoryTrace() as memtrace:
        for step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="evaluating Epoch", dynamic_ncols=True)):
            ...
            with torch.no_grad():
                ...
                labels = batch["labels"][:, 0]  # (batch_size,)
                predictions = (torch.sigmoid(cls_logits).squeeze(-1) > 0.5).long()

                correct = (predictions == labels).sum()  # 预测正确的样本数量
                total = labels.numel()  # 当前 batch 的总样本数
                ...
                cls_loss += position_loss.detach().float()  
    ...
    cls_epoch_loss = cls_loss / len(eval_dataloader)
    if train_config.enable_fsdp:
        eval_epoch_loss = eval_epoch_loss/world_size
        cls_epoch_loss = cls_epoch_loss/world_size
    eval_ppl = torch.exp(eval_epoch_loss)

    global_accuracy = (total_correct / total_samples).detach().float().item()
    global_precision = (total_tp / (total_tp + total_fp + 1e-8)).detach().float().item()  # 避免除零
    global_recall = (total_tp / (total_tp + total_fn + 1e-8)).detach().float().item()
    global_f1 = 2 * global_precision * global_recall / (global_precision + global_recall + 1e-8)
    total_positive = (total_tp + total_fn).detach().cpu().int().item()
    total_samples = total_samples.detach().cpu().int().item()

    # Print evaluation metrics
    if train_config.enable_fsdp:
        if local_rank==0:
            print(f" {eval_ppl=} {eval_epoch_loss=} {cls_epoch_loss=}")
            print(f"{total_positive}/{total_samples} {global_accuracy=} {global_precision=} {global_recall=} {global_f1=}")
    else:
        print(f" {eval_ppl=} {eval_epoch_loss=} {cls_epoch_loss=}")
