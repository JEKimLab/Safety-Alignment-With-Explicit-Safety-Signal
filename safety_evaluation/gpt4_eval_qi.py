...
def main(
    ...
    **kwargs
    ...
    question_dataset, prefill_attacks = question_read(anchor_file)

    for qam in QAMpairs:
        if qam["Q"] in question_dataset:
            index = question_dataset.index(qam['Q'])
            qam["Q"] = qam["Q"] + f" {prefill_attacks[index]}"
            qam["A"] = qam["A"][len(prefill_attacks[index]):]
